...
function checkPose(prediction, video) {
    ...
    if (prob > 0.8 && !explosionActive) {
        ...
        switch(poseNumber) {
            case '1':
                if (time >= 4 && time <= 5 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '2':
                if (time >= 8 && time <= 9 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '3':
                if (time >= 24 && time <= 26 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                
                break;
            case '4':
                if (time >= 26 && time <= 27 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '5':
...
async function playInstructionVideo() {
    ...
    if (model) {
        ...
        console.log("https://teachablemachine.withgoogle.com/models/[...]");
    }
}
