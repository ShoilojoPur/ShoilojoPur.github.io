...
let URL = "https://teachablemachine.withgoogle.com/models/WX8GE5KMb/";
...
function checkPose(prediction, video) {
    ...
    if (prob > 0.8 && !explosionActive) {
        ...
        switch(poseNumber) {
            case '1':
                if (time >= 0.6 && time <= 0.7 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '2':
                if (time >= 0.7 && time <= 0.8 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '3':
             if (time >= 0.15 && time <= 0.16 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '4':
                if (time >= 0.28 && time <= 0.29 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
            case '5':
                if (time >= 0.33 && !poseState.triggered) {
                    triggerExplosion(poseState);
                }
                break;
        }
...
async function playInstructionVideo() {
    ...
    if (model) {
        ...
        console.log("https://teachablemachine.withgoogle.com/models/WX8GE5KMb/");
    }
}
